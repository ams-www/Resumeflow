...
const Templates = (() => {
    ...
    return Object.freeze({
         ...
         * OPTIMIZATION: replaced reduce+push with filter+map for clarity and equivalent performance.
        ...
        previewEntry(item) {
            ...
            const subtitle    = item.subtitle
                ? `<div class="entry-subtitle">${Utils.escapeHtml(item.subtitle)}</div>`
                : '';
            const description = item.description
                ? `<div class="entry-description">${Utils.formatText(item.description)}</div>`
                : '';
